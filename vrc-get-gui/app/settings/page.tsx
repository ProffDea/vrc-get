...
function Settings({
	...
	const [pickUnityHub, unityHubDialog] = useFilePickerFunction(
		environmentPickUnityHub,
	);
	const [pickProjectDefaultPath, projectDefaultDialog] = useFilePickerFunction(
		environmentPickProjectDefaultPath,
	);
	const [pickProjectBackupPath, projectBackupDialog] = useFilePickerFunction(
		environmentPickProjectBackupPath,
	);
	...
	const toggleShowPrereleasePackages = async (
	...
	const reportIssue = async () => {
		...
		url.searchParams.append(
			"os",
			`${await osApi.type()} - ${await osApi.platform()} - ${await osApi.version()} - ${await osApi.arch()}`,
		);
	...
	return (
		<main className="flex flex-col gap-2 flex-shrink overflow-y-auto flex-grow">
			<Card className={"flex-shrink-0 p-4"}>
				...
				<div className={"flex gap-1"}>
					{settings.unity_hub ? (
						...
						<Input
							value={"Unity Hub Not Found"}
							disabled
							className={"flex-auto text-red-900"}
						/>
					)}
					<Button className={"flex-none px-4"} onClick={selectUnityHub}>
			...
			<Card className={"flex-shrink-0 p-4"}>
				...
				<div className="mt-2">
					...
					<div className={"flex gap-1"}>
						<Input
							className="flex-auto"
							value={settings.project_backup_path}
							disabled
						/>
						<Button
							className={"flex-none px-4"}
							onClick={selectProjectBackupFolder}
						>
							{tc("general:button:select")}
						</Button>
					</div>
				...
				<div className="mt-2">
					<label className={"flex items-center"}>
						...
						>
							...
							<VGOption value={"zip-store"}>
								{tc("settings:backup:format:zip-store")}
							</VGOption>
							<VGOption value={"zip-fast"}>
								{tc("settings:backup:format:zip-fast")}
							</VGOption>
							<VGOption value={"zip-best"}>
			...
			<Card className={"flex-shrink-0 p-4"}>
				...
				<label className={"flex items-center"}>
					<Checkbox
						checked={settings.show_prerelease_packages}
						onChange={toggleShowPrereleasePackages}
					/>
			...
			<Card className={"flex-shrink-0 p-4"}>
				<label className={"flex items-center"}>
					...
					<VGSelect
						value={tc("settings:langName")}
						onChange={changeLanguage}
						menuClassName={"w-96"}
					>
						{languages.map((lang) => (
							<VGOption key={lang} value={lang}>
			...
			<Card className={"flex-shrink-0 p-4"}>
				...
				<div>
					<Button onClick={() => emit("tauri://update")}>
