...
function findRecommendedUnity(
	...
	return unityVersions.unity_paths.filter(
	...
	return (
		<>
			...
			<DialogFooter>
				...
				<Button onClick={() => doMigrate(false)} color={"red"} className="mr-1">
					{tc("projects:button:migrate copy")}
				</Button>
				<Button onClick={() => doMigrate(true)} color={"red"}>
...
function useMigrationInternal({
	...
	const request = async () => {
		...
		if (unityFound.length == 0) setInstallStatus({ state: "noExactUnity2022" });
		else setInstallStatus({ state: "confirm" });
	};
		...
		try {
			...
			switch (unityFound.length) {
				...
				default:
					...
					// noinspection ES6MissingAwait
					else continueMigrateProjectTo2022(inPlace, selected);
					break;
			}
		} catch (e) {
			console.error(e);
	...
	const continueMigrateProjectTo2022 = async (
		inPlace: boolean,
		unityPath: string,
	) => {
		try {
			...
			dialogBodyForState = (
				<NoExactUnity2022Dialog
					...
					installWithUnityHubLink={
						unityVersions!.install_recommended_version_link
					}
			...
			<>
				...
				{dialogBodyForState == null ? null : (
					<Dialog open handler={nop} className={"whitespace-normal"}>
						<DialogHeader>
							{tc("projects:manage:dialog:unity migrate header")}
						</DialogHeader>
						{dialogBodyForState}
					</Dialog>
				)}
...
function MigrationCopyingDialog() {
	return (
		<DialogBody>
			...
			<Typography>{tc("projects:manage:dialog:do not close")}</Typography>
		</DialogBody>
	);
}
...
function MigrationCallingUnityForMigrationDialog({
	...
	return (
		<DialogBody>
			...
			<Typography>{tc("projects:manage:dialog:do not close")}</Typography>
			<pre
				className={"overflow-y-auto h-[50vh] bg-gray-900 text-white text-sm"}
			>
