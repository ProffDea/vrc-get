...
function PageBody() {
	...
	const packageRowsData = useMemo(() => {
		...
		repositoriesInfo.status,
		repositoriesInfo.data,
		packagesResult.status,
		packagesResult.data,
	...
	const onUpgradeAllRequest = async () => {
		try {
			...
			const changes = await projectUpgradeMultiplePackage(
				projectPath,
				envVersion,
				packages,
			);
	...
	const onInstallBulkRequested = async () => {
		try {
			...
			const changes = await projectInstallMultiplePackage(
				projectPath,
				envVersion,
				packages,
			);
	...
	const onRemoveBulkRequested = async () => {
		try {
			setInstallStatus({ status: "creatingChanges" });
			const changes = await projectRemovePackages(
				projectPath,
				bulkUpdatePackageIds.map(([id, mode]) => id),
			);
			setInstallStatus({
				status: "promptingChanges",
				changes,
				requested: { type: "bulkRemoved" },
			});
		} catch (e) {
			console.error(e);
			setInstallStatus({ status: "normal" });
		...
		<VStack className={"m-4"}>
			...
			<Card className={"flex-shrink-0 p-2 flex flex-row flex-wrap"}>
				<Typography className="cursor-pointer py-1.5 font-bold flex-grow flex-shrink overflow-hidden basis-52">
					{tc(
						...
						{
							components: {
								code: <code className={"bg-gray-200 p-0.5 whitespace-pre"} />,
							},
						},
					)}
				</Typography>
			...
			<main className="flex-shrink overflow-hidden flex">
				<Card className="w-full p-2 gap-2 flex-grow flex-shrink flex shadow-none">
					<div
						className={
							"flex flex-wrap flex-shrink-0 flex-grow-0 flex-row gap-2"
						}
					>
						<Typography className="cursor-pointer py-1.5 font-bold flex-grow-0 flex-shrink-0">
...
function SuggestResolveProjectCard({
	...
	return (
		<Card className={"flex-shrink-0 p-2 flex flex-row items-center"}>
			<Typography className="cursor-pointer py-1.5 font-bold flex-grow-0 flex-shrink overflow-hidden whitespace-normal text-sm">
...
function BulkUpdateCard({
	...
	return (
		<Card
			className={
				"flex-shrink-0 p-2 flex flex-row gap-2 bg-blue-gray-50 flex-wrap"
			}
		>
			{canInstall && (
				<Button disabled={disabled} onClick={bulkInstallAll}>
					{tc("projects:manage:button:install selected")}
				</Button>
			)}
			{canUpgrade && (
				<Button disabled={disabled} onClick={bulkUpgradeAll} color={"green"}>
					{tc("projects:manage:button:upgrade selected")}
				</Button>
			)}
			{canRemove && (
				<Button disabled={disabled} onClick={bulkRemoveAll} color={"red"}>
...
function ProjectChangesDialog({
	...
	const packageChangesSorted =
		changes.package_changes.sort(comparePackageChange);
	...
	return (
		<Dialog open handler={nop} className={"whitespace-normal"}>
					...
					{packageChangesSorted.map(([pkgId, pkgChange]) => {
						if ("InstallNew" in pkgChange) {
							...
							if (
								...
								!changelogUrlTmp.startsWith("http") &&
								!changelogUrlTmp.startsWith("https")
							)
								changelogUrlTmp = null;
								...
								<ListItem key={pkgId}>
									<Typography className={"font-normal"}>
										{tc("projects:manage:dialog:install package", {
											name:
												pkgChange.InstallNew.display_name ??
												pkgChange.InstallNew.name,
									...
									{changelogUrl != null && (
										<Button
											className={"ml-1 px-2"}
											size={"sm"}
											onClick={() => shellOpen(changelogUrl)}
										>
			...
			<DialogFooter>
				...
				<Button onClick={apply} color={"red"}>
...
function comparePackageChange(
	[aName, aChange]: [string, TauriPackageChange],
	[bName, bChange]: [string, TauriPackageChange],
): number {
...
type PackageLatestInfo =
	| { status: "none" }
	| { status: "contains"; pkg: TauriPackage }
	| {
			status: "upgradable";
			pkg: TauriPackage;
	  };
...
const PackageRow = memo(function PackageRow({
	...
	const installLatest = () => {
		...
		const latest =
			pkg.unityCompatible.get(latestVersion) ??
			pkg.unityIncompatible.get(latestVersion);
		if (!latest) return;
		...
		<tr className="even:bg-blue-gray-50/50">
			...
			<td className={`${cellClass} overflow-hidden max-w-80 overflow-ellipsis`}>
				<div className="flex flex-col">
					<Typography className="font-normal">{pkg.displayName}</Typography>
			...
			<td className={`${noGrowCellClass} max-w-32 overflow-hidden`}>
				{pkg.sources.size === 0 ? (
					...
					<Tooltip content={[...pkg.sources].join(", ")}>
						<Typography>{tc("projects:manage:multiple sources")}</Typography>
					</Tooltip>
				)}
			</td>
			<td className={noGrowCellClass}>
				<div className="flex flex-row gap-2 max-w-min">
					{pkg.installed ? (
						<Tooltip content={tc("projects:manage:tooltip:remove packages")}>
							<IconButton variant={"text"} disabled={locked} onClick={remove}>
								<MinusCircleIcon className={"size-5 text-red-700"} />
							</IconButton>
						</Tooltip>
					) : (
						<Tooltip content={tc("projects:manage:tooltip:add package")}>
							<IconButton
								variant={"text"}
								disabled={locked && !!latestVersion}
								onClick={installLatest}
							>
								<PlusCircleIcon className={"size-5 text-gray-800"} />
							</IconButton>
						</Tooltip>
					)}
				</div>
...
const PackageVersionSelector = memo(function PackageVersionSelector({
	...
	return (
		<VGSelect
			value={<PackageInstalledInfo pkg={pkg} />}
			className={`border-blue-gray-200 ${
				pkg.installed?.yanked ? "text-red-700" : ""
			}`}
			onChange={onChange}
			disabled={locked}
		>
			{versionNames.map((v) => (
				<VGOption key={v} value={v}>
					{v}
				</VGOption>
			))}
			...
			{incompatibleNames.length > 0 && (
				<Typography className={"text-sm"}>
					{tc("projects:manage:incompatible packages")}
				</Typography>
			)}
			...
			return (
				<Tooltip content={tc("projects:manage:tooltip:upgrade package")}>
					<Button
						...
						color={"green"}
						className={
							"text-left px-2 py-1 w-full h-full font-normal text-base normal-case"
						}
						disabled={locked}
	...
	return (
		<HNavBar className={className}>
			...
			<Menu>
				<ButtonGroup>
					<Button
						onClick={() => openUnity.openUnity(projectPath, unityVersion)}
						className={"pl-4 pr-3"}
					>
				...
				<MenuList>
					...
					<MenuItem onClick={onRemove} className={"bg-red-700 text-white"}>
