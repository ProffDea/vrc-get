...
export function environmentAddProjectWithPicker() {
	return invoke()<TauriAddProjectWithPickerResult>(
		"environment_add_project_with_picker",
	);
}

export function environmentRemoveProject(
	listVersion: number,
	index: number,
	directory: boolean,
) {
	return invoke()<null>("environment_remove_project", {
		listVersion,
		index,
		directory,
	});
...
export function environmentRemoveProjectByPath(
	path: string,
	directory: boolean,
) {
	return invoke()<null>("environment_remove_project_by_path", {
		path,
		directory,
	});
}
...
export function environmentCopyProjectForMigration(sourcePath: string) {
	return invoke()<string>("environment_copy_project_for_migration", {
		sourcePath,
	});
}

export function environmentSetFavoriteProject(
	listVersion: number,
	index: number,
	favorite: boolean,
) {
	return invoke()<null>("environment_set_favorite_project", {
		listVersion,
		index,
		favorite,
	});
...
export function environmentPickProjectDefaultPath() {
	return invoke()<TauriPickProjectDefaultPathResult>(
		"environment_pick_project_default_path",
	);
}

export function environmentPickProjectBackupPath() {
	return invoke()<TauriPickProjectBackupPathResult>(
...
export function environmentDownloadRepository(
	url: string,
	headers: { [key: string]: string },
) {
	return invoke()<TauriDownloadRepository>("environment_download_repository", {
		url,
		headers,
	});
}

export function environmentAddRepository(
	url: string,
	headers: { [key: string]: string },
) {
	return invoke()<TauriAddRepositoryResult>("environment_add_repository", {
		url,
		headers,
	});
}
...
export function environmentProjectCreationInformation() {
	return invoke()<TauriProjectCreationInformation>(
		"environment_project_creation_information",
	);
}

export function environmentCheckProjectName(
	basePath: string,
	projectName: string,
) {
	return invoke()<TauriProjectDirCheckResult>(
		"environment_check_project_name",
		{ basePath, projectName },
	);
}

export function environmentCreateProject(
	basePath: string,
	projectName: string,
	template: TauriProjectTemplate,
) {
	return invoke()<TauriCreateProjectResult>("environment_create_project", {
		basePath,
		projectName,
		template,
	});
...
export function projectInstallPackage(
	projectPath: string,
	envVersion: number,
	packageIndex: number,
) {
	return invoke()<TauriPendingProjectChanges>("project_install_package", {
		projectPath,
		envVersion,
		packageIndex,
	});
...
export function projectInstallMultiplePackage(
	projectPath: string,
	envVersion: number,
	packageIndices: number[],
) {
	return invoke()<TauriPendingProjectChanges>(
		"project_install_multiple_package",
		{ projectPath, envVersion, packageIndices },
	);
}

export function projectUpgradeMultiplePackage(
	projectPath: string,
	envVersion: number,
	packageIndices: number[],
) {
...
export function projectResolve(projectPath: string) {
	return invoke()<TauriPendingProjectChanges>("project_resolve", {
		projectPath,
	});
}

export function projectRemovePackages(projectPath: string, names: string[]) {
	return invoke()<TauriPendingProjectChanges>("project_remove_packages", {
		projectPath,
		names,
	});
}

export function projectApplyPendingChanges(
	projectPath: string,
	changesVersion: number,
) {
	return invoke()<null>("project_apply_pending_changes", {
		projectPath,
		changesVersion,
	});
}
...
export function projectCallUnityForMigration(
	channel: string,
	projectPath: string,
	unityPath: string,
) {
	return invoke()<AsyncCallResult<string, TauriCallUnityForMigrationResult>>(
		"project_call_unity_for_migration",
		{ channel, projectPath, unityPath },
	);
}
...
export function projectCreateBackup(channel: string, projectPath: string) {
	return invoke()<AsyncCallResult<null, null>>("project_create_backup", {
		channel,
		projectPath,
	});
}
...
export type TauriCreateProjectResult =
	| "AlreadyExists"
	| "TemplateNotFound"
	| "Successful";
export type TauriPackage = {
	...
	legacy_packages: string[];
	is_yanked: boolean;
} & { env_version: number; index: number; source: TauriPackageSource };
export type TauriUserRepository = {
	id: string;
	url: string | null;
	display_name: string;
};
export type TauriPackageSource =
	| "LocalUser"
...
export type TauriProjectDetails = {
	unity: [number, number] | null;
	unity_str: string | null;
	installed_packages: [string, TauriBasePackageInfo][];
	should_resolve: boolean;
};
export type TauriPickProjectBackupPathResult =
	| "NoFolderSelected"
	| "InvalidSelection"
	| "Successful";
export type TauriProjectDirCheckResult =
	| "InvalidNameForFolderName"
	| "MayCompatibilityProblem"
	| "WideChar"
	| "AlreadyExists"
	| "Ok";
export type TauriPickProjectDefaultPathResult =
	| { type: "NoFolderSelected" }
	| { type: "InvalidSelection" }
	| { type: "Successful"; new_path: string };
export type TauriDownloadRepository =
	| { type: "BadUrl" }
	| { type: "Duplicated" }
	| { type: "DownloadError"; message: string }
	| { type: "Success"; value: TauriRemoteRepositoryInfo };
...
export type TauriCallUnityForMigrationResult =
	| { type: "ExistsWithNonZero"; status: string }
...
export type AsyncCallResult<P, R> =
	| { type: "Result"; value: R }
	| { type: "Started" }
	| { type: "UnusedProgress"; progress: P };
export type TauriPickUnityResult =
	| "NoFolderSelected"
	| "InvalidSelection"
	| "AlreadyAdded"
	| "Successful";
...
export type TauriAddProjectWithPickerResult =
	| "NoFolderSelected"
	| "InvalidSelection"
	| "AlreadyAdded"
	| "Successful";
export type TauriProjectType =
	| "Unknown"
	| "LegacySdk2"
	| "LegacyWorlds"
	| "LegacyAvatars"
